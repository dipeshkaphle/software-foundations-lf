...
Definition conj_fact : forall P Q R, P /\ Q -> Q /\ R -> P /\ R.
  ...
  intros.
  apply conj.
  - apply (proj1 P Q). assumption.
  - apply (proj2 Q R). assumption.
Defined.
...
Definition ex_falso_quodlibet' : forall P, False -> P := fun P fls => match fls with end.
(** [] *)
...
Proof.
  intros.
  specialize H with (P:=(fun a => eq x a)).
  simpl in H.
  apply H.
  apply eq_refl.
Qed.
...
Definition curry : forall P Q R : Prop,
    ((P /\ Q) -> R) -> (P -> (Q -> R)) :=
  fun P Q R f1 =>
    (fun p => (fun q => f1 (conj p q))).

Definition uncurry : forall P Q R : Prop,
    (P -> (Q -> R)) -> ((P /\ Q) -> R) :=
  fun P Q R f => (fun pq => match pq with
                | conj p q => (f p) q
                end
              ).
...
Proof.
  ...
  apply H.
  split.
  - intros [Hp | Hq]. apply (or_intror Hp). apply (or_introl Hq).
  - intros [Hq | Hp]. apply (or_intror Hq). apply (or_introl Hp).
Qed.
(** [] *)
...
Proof.
  ...
  intros.
  apply (pe_implies_true_eq H P ) in pf1 as H2.
  unfold propositional_extensionality  in H.
  specialize H with (P:=True) (Q:=P).
  destruct H.
  - split. rewrite H2. intros. assumption.
    rewrite H2. intros. assumption.
