...
(* *)
(* P -> ~~P *)
(* - By contrapositive: ~(~~P) -> ~P.    *)
(* - By contradiction: Assume ~~P.   *)
(* - Premise is:~(~~P) *)
(* - assumption can't be true because of premise. So, qed by contradiction. *)
(**)
...
(* By Contradiction:  Assume ~(~(P /\ ~P)). *)
(* By double negation elimination:  (P /\ ~P). *)
(* Always false by definition of and & not.) *)
(* Qed.) *)
...
Proof.
  ...
  intros h.
  split.
  - unfold not. intros Hp. unfold not in h. apply h. left. apply Hp.
  - unfold not. intros Hq. unfold not in h. apply h. right. apply Hq.
Qed.
(** [] *)
...
Proof.
  ...
  unfold not.
  intros [x HNPx].
  apply HNPx in HPx as Impossible.
  destruct Impossible.
Qed.
(** [] *)
...
Theorem n_leb_0_implies_n_0:  forall n, n<=? 0 = true -> n = 0.
Proof.
...
Theorem n_eqb_m_implies_n_m:  forall n m, n =? m = true -> n = m.
Proof.
  induction n as [|n' IHn].
  + simpl. destruct m as [|m']. reflexivity. discriminate.
  + simpl. destruct m as [|m']. discriminate. intros h. apply IHn in h. f_equal. apply h.
Qed.

Theorem n_leb_m_implies_n_le_m : forall n m, n<=? m =true -> n <= m.
Proof.
  intros n m h.
...
Proof.
  ...
  exists (m-n).
  Search (?x + ( ?y - ?x )).
  rewrite Arith_base.le_plus_minus_r_stt. reflexivity.
  apply n_leb_m_implies_n_le_m.
  apply H.
Qed.


Theorem n_leb_succ: forall a b, a <=? a + S b = true.
Proof.
...
Proof.
  ...
  - intros l'' h. simpl. split.
    + intros h'. right. apply h'.
    + intros [Falsehood| goal]. destruct Falsehood. apply goal.
  - intros l'' a. simpl. split.
    + intros [ Heq | HRest ].
      * left. left. apply Heq.
      * apply IH in HRest. destruct HRest as [Hl'|Hl''].
        ** left. right. apply Hl'.
        ** right. apply Hl''.
    + intros [[Heq | Hrest] | H2].
      * left. apply Heq.
      * right. apply IH.  left. apply Hrest.
...
Proof.
  ...
  - split.
    ...
    + simpl. intros h x Falsehood. destruct Falsehood.
  - split.
    + intros h.
      simpl. split.
      * apply h. simpl. left. reflexivity.
      * apply IHl. simpl in h. intros x h'.
        apply h. right. apply h'.
    + simpl.
      intros h x.
      intros [Hhd_x| Hrest].
      * rewrite Hhd_x in h. apply h.
      * apply IHl in Hrest. apply Hrest. apply h.
Qed.
(** [] *)
...
(*TODO: Check again because some wizardry is going on in that destruct lol*)
...
Proof.
  intros n.
  induction n as [|n' [k Hk]].
  - exists 0. simpl. reflexivity.
  - Search even. rewrite even_S.  destruct (even n') eqn:E.
    + simpl. exists k. rewrite Hk. reflexivity.
    + simpl. exists (S k). simpl. rewrite Hk. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros [Hb1 Hb2]. unfold andb. rewrite Hb1. apply Hb2.
Qed.
...
Proof.
  intros b1 b2.
  split.
  - intros h. destruct b1 eqn:Eb. simpl in h.
    + left. reflexivity.
    + simpl in h. rewrite h. right. reflexivity.
  - intros [Hb1 | Hb2].
    + rewrite Hb1. reflexivity.
    + rewrite Hb2. destruct b1 eqn:E. reflexivity. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros x y.
  split.
  - intros h. unfold not. intros Hxy. rewrite Hxy in h. rewrite eqb_refl in h. discriminate h.
  - unfold not.
    intros h.
    destruct (x =? y) eqn:E.
    apply eqb_eq in E.
...
Fixpoint eqb_list {A : Type} (eqb : A -> A -> bool)
  ...
  | [],hd::tl => false
  | hd::tl , [] => false
  | hd'::tl', hd''::tl'' => andb ( eqb hd' hd''  ) (eqb_list eqb tl' tl'')
end.
...
Proof.
  ...
  + generalize dependent l1.
    induction l2 as [|h2 tl2 IHl2].
    * destruct l1 as [|h1 tl1].
      - reflexivity.
      - intros h'. discriminate h'.
    * destruct l1 as [|h1 tl1] eqn:E.
      - simpl. intros h'. discriminate h'.
      - simpl. intros h'.
        apply andb_true_iff in h' as [Eqh Eqtl].
        apply h in Eqh. rewrite <- Eqh.
        apply IHl2 in Eqtl.
        rewrite <- Eqtl.
        reflexivity.
  + generalize dependent l1.
    ...
    * destruct l1 as [|h1 tl1].
      - intros h'. discriminate h'.
      - intros h'. injection h' as H_head H_tail.
        apply h in H_head.
        apply IHl2 in H_tail.
        simpl. rewrite H_head. rewrite H_tail.
        reflexivity.
Qed.
...
Proof.
  ...
  - simpl. split. intros h. apply I. intros h. reflexivity.
  - simpl. split.
    + destruct (test h) eqn:E.
      * intros h'. apply IHl in h'. split.
        -- reflexivity.
        -- apply h'.
      * intros h'. discriminate h'.
    + destruct (test h) eqn:E.
      * intros [ _ HAll ]. apply IHl in HAll. apply HAll.
      * intros [Falsehool _]. apply Falsehool.
Qed.
...
Theorem rev_append_l1_l2_is_rev_append_l1_empty_app_l2:  forall X l2 l1, @rev_append X l1 l2 = @rev_append X l1 [] ++ l2.
Proof.
  intros X l2 l1.
  generalize dependent l2.
  induction l1 as [|h tl IHl] .
  + reflexivity.
  + simpl.
    intros l2.
    rewrite IHl.
...
Theorem tr_rev_tl : forall X x l, @tr_rev X (x::l) = @tr_rev X l ++ [x] .
Proof.
  intros X x l.
  generalize dependent x.
  induction l as [|hd tl IHl].
  - simpl. unfold tr_rev. simpl. reflexivity.
  - unfold tr_rev. simpl. unfold tr_rev in IHl.
    intros x.
    rewrite rev_append_l1_l2_is_rev_append_l1_empty_app_l2.
    rewrite ( rev_append_l1_l2_is_rev_append_l1_empty_app_l2 X ([hd]) tl).
    rewrite <-app_assoc.
    reflexivity.
Qed.

Theorem tr_rev_correct_fully_spelled_out: forall X l, @tr_rev X l = @rev X l.
Proof.
  induction l as [|h tl IHl].
  + reflexivity.
  + simpl. rewrite <- IHl. rewrite tr_rev_tl. reflexivity.
Qed.
...
Proof.
  ...
  apply tr_rev_correct_fully_spelled_out.
Qed.
(** [] *)
...
(* TODO: Continue from here, I'm beat *)
...
Proof.
  ...
  intros excluded_middle X P.
  unfold not.
  unfold not in excluded_middle.
  intros h.
  intros x.
  specialize excluded_middle with (P:=(P x)).
  destruct excluded_middle as [Px | NPx] eqn:E.
  - apply Px.
  - exfalso. apply h. exists x. apply NPx.
Qed.
(** [] *)
...
(* FILL IN HERE
    TODO: Not currently sure how to do this  *)
(*    [] *)
