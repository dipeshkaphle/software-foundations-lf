...
Proof.
  ...
  - reflexivity.
  - destruct m eqn:Em.
    * rewrite add_0_r. simpl. rewrite PeanoNat.Nat.add_1_r. reflexivity.
    * simpl. rewrite IHn'. reflexivity.
Qed.
...
Proof.
  ...
  - destruct m eqn:Em.
    ...
    * simpl. rewrite <- plus_n_Sm. rewrite <- plus_n_Sm. rewrite IHn'. reflexivity.
Qed.
...
Proof.
  ...
  * simpl.
    assert (H: forall n k, n * (1 + k) = n + n * k).
    { intros n' k'.
...
(* Had forgotten about mult_n_Sm so did look at https://stackoverflow.com/questions/34758527/proving-that-multiplication-is-commutative *)
Theorem mul_comm_try_2 : forall m n : nat,
  m * n = n * m.
Proof.
  intros m n.
  induction m as [|m' IHm'].
  ...
  * simpl.
    rewrite IHm'.
    assert (H: forall a b, a * (1 + b) = a + a * b).
    {
        intros a b.
        induction a as [| a' IHa'].
        - simpl. reflexivity.
        - rewrite <- ( mult_n_Sm ( S a' ) _). rewrite add_comm. reflexivity.
    }
    rewrite H.
    reflexivity.
Qed.
...
Proof.
   ...
   - intros H. simpl. rewrite IHp'. reflexivity. rewrite H. reflexivity.
Qed.
...
Proof.
  intros n m p.
  induction p as [| p' IHp'].
  - rewrite <- mult_n_O. rewrite mult_n_0_m_0. reflexivity.
  - rewrite <- mult_n_Sm.
    rewrite <- (mult_n_Sm n p').
    rewrite (add_comm _ (m * S p')).
    rewrite <- (mult_n_Sm m p').
    rewrite <- (add_assoc _ m _).
    rewrite (add_comm m _).
    rewrite <- (add_assoc _ n m).
    rewrite (add_assoc (m * p') (n * p') _).
    rewrite IHp'.
    rewrite (add_comm (n * p') (m * p')).
    reflexivity.
Qed.
...
Proof.
  ...
  induction n as [| n' IHn'].
  - simpl. reflexivity.
  - rewrite (mul_comm (S n' * m) p).
    rewrite (mul_comm _ m).
    rewrite <- mult_n_Sm.
    rewrite mul_comm.
    rewrite <- mult_n_Sm.
    rewrite (mul_comm p (m * n' + m)).
...
Theorem add_n_plus_Sm : forall m n, m + S n = S (m + n).
Proof.
  intros m n.
  induction m as [|m' IHm'].
  - simpl. reflexivity.
...
Theorem double_bin_equals_double : forall b, bin_to_nat ( double_bin b ) = double (bin_to_nat b) .
Proof.
  induction b as [|b IHb0 | b IHb1].
  - simpl. reflexivity.
  - simpl. rewrite add_0_r. rewrite add_0_r. rewrite <- double_plus. reflexivity.
  - simpl. rewrite add_0_r. rewrite add_0_r. rewrite <- double_plus. rewrite add_n_plus_Sm. rewrite <- double_plus. reflexivity.
Qed.
...
Proof.
  ...
  - simpl.
    rewrite add_0_r.
    rewrite <- double_plus.
    rewrite <- double_bin_equals_double.
    Admitted.
(* Th *)
...
(* Tried to prove this , the problematic thing might be the B0 Z case  *)
...
Lemma natToBin_double_is_doubleBin : forall n, nat_to_bin (double n) = double_bin (nat_to_bin  n).
Proof.
  induction n as [|n' IHn'].
  - simpl. reflexivity.
...
Lemma B1_prefix_doubles_and_adds_one: forall b', B1 b' = incr ( double_bin b' ) .
Proof.
  induction b'.
  - simpl. reflexivity.
  - simpl. reflexivity.
...
Proof.
   ...
   - simpl.
     rewrite add_0_r.
     rewrite <-double_plus.
     rewrite natToBin_double_is_doubleBin.
     rewrite IHb0.
     reflexivity.
   - simpl.
     rewrite add_0_r.
     ...
     rewrite natToBin_double_is_doubleBin.
     rewrite IHb1.
     assert (B1_prefix_doubles_and_adds_one: forall b', B1 b' = incr ( double_bin b' ) ).
     {
        induction b'.
        - simpl. reflexivity.
        - simpl. reflexivity.
     ...
     rewrite B1_prefix_doubles_and_adds_one.
     reflexivity.
Qed.
