...
Proof.
  induction b; try (simpl;
                    ...
                    try (repeat (rewrite IHb));
                    try (repeat (rewrite IHb1));
                    try (repeat (rewrite IHb2));
                    reflexivity );
    try reflexivity.
Qed.
...
(* I am familiar with writing inference rules, it's easy but time-consuming, so
skipping *)
...
Inductive bevalR: bexp -> bool -> Prop :=
  ...
  | E_BNot ( b: bexp ) (bv1: bool) : bevalR b bv1 -> bevalR (BNot b) (negb bv1)
...
Proof.
  ...
  - intros.
    ...
    induction b; intros ; simpl in *; subst; constructor;
