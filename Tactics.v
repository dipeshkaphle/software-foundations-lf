...
(* FILL IN HERE *)
(* - apply will try to match the goal exactly but rewrite is just arbitrary substitution of one term with other representation of the same term.
...
(** Another handy tactic for fiddling with hypotheses is [
...
Proof.
  induction n as [ | n' ].
  - intros m Heq. destruct m as [|m'].
    * reflexivity.
    * discriminate.
  - intros m Heq. destruct m as [|m'].
    * simpl in Heq. discriminate Heq.
    * simpl in Heq. f_equal. apply IHn' in Heq. apply Heq.
Qed.
(** [] *)
...
(* TODO: I don't want to write this for now, proving in Coq is more fun *)
...
Proof.
  ...
  - intros n' H.
    rewrite <-H.
    simpl. rewrite ( IHtl (length tl)). reflexivity. reflexivity.
Qed.
...
(*Took a lot of time and had to take some help because I forgot I could destruct a tuple and was stuck trying to make it proceed even though the goal was making sense. *)
...
Proof.
  intros X Y.
  induction l as [|hd tl IHtl].
  - intros l1 l2 h. simpl in h. injection h as hl1 hl2. symmetry in hl1, hl2. rewrite hl1, hl2. reflexivity.
  - intros l1 l2. destruct hd as [frst scnd].
    simpl. destruct (split tl) as [tl_fst tl_snd].
    intros h.
    injection h as hl1 hl2.
    rewrite <-hl1 , <-hl2.
    simpl.
    rewrite ( IHtl tl_fst tl_snd). reflexivity. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct (f true) eqn:Eftrue.
    + rewrite Eftrue. rewrite Eftrue. reflexivity.
  ...
  - destruct (f false) eqn:Effalse.
    + destruct (f true) eqn:Eftrue.
      * apply Eftrue.
      * apply Effalse.
    + rewrite Effalse. rewrite Effalse. reflexivity.
Qed.
(** [] *)
...
(** **** Exercise: 3 stars, advanced, optional (eqb_sym_informal)
   ...
   Proof: *)
   (* Proof in Coq is more fun so skipping.
...
Proof.
  ...
  - intros l2 h. simpl in h. simpl. destruct l2 as [|hd2 tl2].
    + reflexivity.
    + discriminate h.
  - intros l2 h. simpl. destruct l2 as [|hd2 tl2].
    + simpl in h. discriminate h.
    + simpl in h. simpl. rewrite IHtl1. reflexivity. injection h as Hlen. apply Hlen.
Qed.
...
Proof.
  intros X test.
  induction l as [|hd tl IHtl].
  - simpl. intros lf h. discriminate .
  - intros lf. simpl. destruct (test hd) eqn:E.
    + intros h. injection h as H1. rewrite <- H1. apply E.
    + intros h. rewrite ( IHtl lf). reflexivity. apply h.
Qed.
(** [] *)
...
Proof.
  induction l as [|hd tl IHtl].
  - reflexivity.
  - assert (existsb'_distributes: existsb' test (hd::tl) = orb (test hd) (existsb' test tl)). {
      unfold existsb'.
      simpl.
  ...
  simpl. rewrite existsb'_distributes. destruct (test hd) eqn:E.
    + reflexivity.
    + simpl. apply IHtl.
Qed.
