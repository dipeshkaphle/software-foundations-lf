...
Fixpoint nonzeros (l:natlist) : natlist :=
  ...
  | nil => nil
  | hd :: tl =>
      let rem := nonzeros tl in
      if (hd =? 0) then rem else hd::rem
end.
...
Fixpoint oddmembers (l:natlist) : natlist :=
  ...
  | nil => nil
  | hd :: tl =>
      let rem := oddmembers tl in
      if (Nat.odd hd) then hd::rem else rem
end.
...
Fixpoint alternate (l1 l2 : natlist) : natlist :=
  match l1, l2 with
  | nil, nil => nil
  | cons h1 t1, nil => cons h1 t1
  | nil, cons h2 t2  => cons h2 t2
  | cons h1 t1, cons h2 t2 => cons h1 (cons h2 (alternate t1 t2))
end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | nil => 0
  | hd::tl => (if (hd =? v) then 1 else 0) + (count v tl)
end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with
  | nil => nil
  | hd::tl => if (hd =? v) then tl else hd::(remove_one v tl)
end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | hd::tl => if (hd =? v) then (remove_all v tl) else hd::(remove_all v tl)
end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  ...
  | nil => true
  | hd::tl =>
      let one_hd_removed_s2 := remove_one hd s2 in
      if member hd s2(*Nothing was removed*)
      then included tl one_hd_removed_s2
      else false
end.
...
Theorem add_inc_count : forall v s, count v ( add v s ) = S ( count v s ).
Proof.
  destruct v as [|v'] .
  - simpl. reflexivity.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  match l1,l2 with
  | nil , nil => true
  | hd1::tl1 , hd2::tl2 => if (hd1 =? hd2) then eqblist tl1 tl2 else false
  | _, _ => false
end.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. destruct n'.
    + simpl. Search (?x <=? S ?x). rewrite leb_n_Sn. reflexivity.
...
*)
Theorem bag_count_sum : forall n b1 b2, count n (sum b1 b2) = count n b1 + count n b2.
Proof.
  intros n b1 b2.
  induction b1 as [|n' b1'].
  - simpl. reflexivity.
  - simpl. destruct (eqb n' n).
