...
Proof.
  simpl.
  intros X l.
  induction l as [|hd tl Itl].
  - reflexivity.
  - simpl. rewrite Itl. reflexivity.
Qed.
...
Proof.
  ...
  - simpl. rewrite Itl. rewrite app_assoc. reflexivity.
Qed.
...
Proof.
  induction l as [| hd tl Itl].
  - reflexivity.
  - simpl. rewrite rev_app_distr. rewrite Itl. reflexivity.
Qed.
...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
    | [] => ([],[])
    | (x,y)::tl =>
      let tl_split := split tl in
      ( x::(fst tl_split) , y::(snd tl_split) )
end.
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => andb (even x) (negb (leb x 7 )) ) l.
...
Theorem map_distr_over_app : forall (X Y: Type) (f: X -> Y ) (l1: list X) (l2 : list X),
             map f (l1 ++ l2) = ( map f l1 ) ++ (map f l2).
Proof.
  intros X Y f l1 l2.
  induction l1 as [|hd tl IHt].
  - reflexivity.
...
Proof.
  intros X Y.
  intros mapfn l.
  induction l as [|hd tl IHt].
  - reflexivity.
  - simpl. rewrite map_distr_over_app. rewrite IHt. reflexivity.
Qed.
...
Fixpoint filter' (X:Type) (test: X->bool) (l:list X) : list X :=
  ...
  | [] => []
  | h :: t =>
    if test h then h :: (filter' _ test t)
...
Definition partition_using_fold {X: Type} (test: X -> bool) (l : list X) : list X * list X :=
  fold (fun cur acc  =>  if test cur then (cur::(fst acc), snd acc) else (fst acc, cur::(snd acc)) ) l ([],[]).

Compute partition_using_fold odd [1;2;3;4;5].
...
Theorem nth_error_for_n_eq_len_is_none : forall X l n,
...
Definition exp (n m : cnat) : cnat :=
  fun X f x =>
    let fn := fun (f': X -> X) (x': X) => n X f' x' in
    ( m _ fn (one X f) ) x.

Compute ((exp two two)) nat S 0 .

Compute ((exp three two)) nat S 0 .
Compute ((exp three three)) nat S 0 .
